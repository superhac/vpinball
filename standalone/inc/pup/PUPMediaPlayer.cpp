...
#endif
   m_pps = new PinSound(nullptr);
   m_pps->StreamInit(44100, 2, 0.0f);
   m_running = false;
   m_paused = false;
}
   ...
   while (true) {
      {
         ...
         m_pps->StreamVolume(m_volume / 100.0f);
      }
...
void PUPMediaPlayer::HandleAudioFrame(AVFrame* pFrame)
{
   ...
   m_pps->StreamUpdate(pBuffer, resampledDataSize);
